...
/// 与 Flutter [kDebugMode] 等价，但不依赖 `package:flutter`（便于 `dart run` 冒烟）。
const bool _kAgentDebugLog = bool.fromEnvironment('dart.vm.product')
    ? false
    : true;

void _agentDebugLog(String message) {
  // ignore: avoid_print
  print(message);
}
...
abstract final class OtlpFlutter {
  ...
  static OtlpAgent? init({
    ...
    String defaultServiceVersion = '1.0.0',
    Duration flushInterval = const Duration(seconds: 5),
    bool logConfig = _kAgentDebugLog,
  }) {
    ...
    if (logConfig) {
      _agentDebugLog(
        '[skywalking_dart] endpoint=${config.otlpEndpoint} '
