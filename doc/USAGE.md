...
在 `pubspec.yaml` 中通过 **Git** 或 **path** 添加 `skywalking_dart` 依赖。
...
$env:OTEL_SERVICE_NAME = "dart-otlp-verify"
...
OAP 加载 `dart-otlp` 规则并重启后，在 **Metrics inspect** 中通过 **+ add metric** 选择 **MAL-OTEL → dart-otlp**，实体选 `xt-open-app`（或你的 `OTEL_SERVICE_NAME`）。常见指标：
...
> Inspect 横轴为时间桶（分钟级）；无数据时先跑 `dart run bin/verify_otlp.dart` 或真实 App 产生 OTLP，并确认 OAP 已启用 `receiver-otel` 与 `dart-otlp` 规则。
...
| 对比项 | Java SkyWalking Agent | skywalking_dart |
|--------|----------------------|-------------------|
